 ...
 */
double Carton::Volume() const { return length * width_ * height_; }
/**
 ...
 */
double Carton::LengthPlusGirth() const { return length + Girth(); }
...
void Carton::WriteData(std::ostream& out) const {
    out << length<< " " << width_ << " " << height_ << endl;
}
/**
...
void Carton::WriteCarton(std::ostream& out) const {
    out << "A carton with length " << length << ", width " << width_
...
/**
 * @brief Outputs the word Carton followed by Weight .....
 *
 */

void Carton::Display(std::ostream& out) const {

    out << "Carton : " << get_weight() << "lbs. " << get_length() << " x "<<get_width();
}